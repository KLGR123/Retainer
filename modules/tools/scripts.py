...
def write_gameplay_script(content: dict) -> str:
    """将游戏玩法写入到指定的 .json 文件中，content 是游戏玩法的内容，为一个字典；
    如果文件此时还不存在，则该工具自动创建文件并写入游戏玩法；如果文件已经存在，该工具覆盖式写入玩法说明。"""

    file_path = os.path.join("assets/scripts", "游戏玩法.json")
...
def read_entire_planning() -> str:
    ...
    用于快速了解现阶段用户对游戏玩法和场景搭建的策划。"""
    
    planning_content = ""
    scene_file_path = os.path.join("assets/scripts", "场景搭建.json")
    gameplay_file_path = os.path.join("assets/scripts", "游戏玩法.json")
